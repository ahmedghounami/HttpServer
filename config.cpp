			   ...
			   server_config &config)
{
	...
	else if (key == "error_page")
	{
		...
		ss >> error_code;
		ss >> error_page;
		if (error_code.empty() || !is_digit(error_code) || error_page.empty() || std::atof(error_code.c_str()) < 100 || std::atof(error_code.c_str()) > 599)
...
void server::parse_config(std::string config_file)
{
	...
	while (std::getline(file, line))
	{
		...
		if (line == "}")
		{
			...
			else if (servers[i].server_names.empty() || servers[i].host.empty() ||
	...
	// if (!stack.empty() || config.server_name.empty() || config.host.empty()
	// || config.ports.empty() || config.path.empty() || config.index.empty() ||
	...
	for (unsigned int i = 0; i < servers.size(); i++)
	{
		...
		for (unsigned int j = 0; j < servers[i].server_names.size(); j++)
			std::cout << "server_name: " << servers[i].server_names[j] << std::endl;
